...
This, in a nutshell, is the fundamental problem of causal inference. It's simple. Deceptively so, because the real world is a mess. So we have assumptions to make. But in return we get something quite extraordinary: A rigorous, tried-and-tested framework for thinking about cause-and-effect-relationships.
...
Given this DAG, then, we should not expect a *direct* causal influence of $X$ on $Y$; they are only associated through the confounding backdoor path $X \leftarrow Z \rightarrow Y$. By statistically adjusting for $Z$, we'd block the backdoor path, making $X$ and $Y$ independent from each other. 
...
glm(Y ~ X, data = forkdat) # bX is non-zero; confounded

glm(Y ~ X + Z, data = forkdat) # bX is zero; de-confounded
...
If we're interested in estimating the *total effect* of $X$ on $Y$, we would not adjust for $Z$, since adjusting for $Z$ would block the causal path. However, say we hypothesized a *direct effect* of $X$ on $Y$ in addition to the mediating path through $Z$. If we were interested in estimating only that direct effect, we would have to adjust for $Z$, since $Z$ would otherwise confound that path. 
...
glm(Y ~ X, data = pipedat) # bX is 0.5*0.5; total effect

glm(Y ~ X + Z, data = pipedat) # bX is zero; direct effect
...
glm(Y ~ X, data = colldat) # bX is 0; X and Y non-associated

glm(Y ~ X + Z, data = colldat) # bX is non-zero; X and Y associated
...
There are many pedagogical resources that discuss in more depth the promises and pitfall of statistical adjustment for causal inference [e.g., @achen2005let; @cinelli2020crash; @lubke2020we; @rohrer2018thinking; @westreich2013table; @wysocki2022statistical]. But, for now, I focus on three critical take-home messages:
...
This fundamental insight stands in contrast to common practice in many empirical sciences, where a table (often Table 2 of an empirical report) with all estimated regression coefficients is often presented -- implicitly or explicitly -- as equal causal effects. This is bad practice as it potentially misleads the reader to interpret coefficients of the control variables causally. Accordingly, this malpractice has been given a name, the "Table 2 Fallacy" [@westreich2013table], to raise awareness about the potential pitfalls.

### Causes are not in the data
Another critical take-home is that causes are not in the data. For instance, it's not possible from the data alone to distinguish between a pipe and a fork , because they have similar statistical implications. Therefore it follows that a DAG cannot be built on the basis of a set of regressions. Instead, regressions can be used to *test* the implications of a DAG or a set of DAGs [for a discussion of this point in the wild, @purzycki_coding_2022]. All this means that variable selection -- which variables to include in your adjustment set -- cannot be automated; it strictly requires prior knowledge and explicit causal assumptions [@hernan2020causal; @westreich2019epidemiology].

### Average people vs. people on average
In all but the simple cases, regression coefficients yield conditional effect estimates. The *average* person (conditional) vs. people *on average* (marginal). Y \~ X is the marginal estimate. But adjustment is critical for de-confounding. But, adjustment returns only conditional estimates (**to be discussed**).
